...
class Game(object):
    ...
    directions = {curses.KEY_LEFT: (-1, 0),
                  ...
                  'down': (0, 1)}
    rotate_directions = {curses.KEY_UP: 1}
    ...
    def draw_border(self):
        ...
        textpad.rectangle(self.win, 0, 10, 10, 15)
    ...
    def draw_block(self):
        for brick in self.current_block.bricks:
            self.win.addch(int(brick.y), int(brick.x), brick.char)

    def redraw_block(self, action, *args):
        # clear block
        for brick in self.current_block.bricks:
            self.win.addch(int(brick.y), int(brick.x), 32)
        # move or rotate
